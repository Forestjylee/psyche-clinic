...
| v1.0.0 | 2026-08-07 | 开罗风格界面彻底重构（§14，规划中）：技术选型结论 Phaser.js，经营场景层 + React UI 覆盖层 + 数据层复用的分层架构，四阶段实施计划 |

---

## 14. 开罗风格界面彻底重构（v1.0.0 · 规划中，暂不动工）

> 本小节为立项文档，仅规划不实施。所有需求与边界在下方明确后，分阶段动工。
...
| 卡通像素/Q 版画风 | 暖色手绘风 | 折中：保持暖色主调，场景/人物像素化或 Q 版化 |
...
| 阶段 | 内容 | 交付物 |
| --- | --- | --- |
| A · 可行性 demo | 诊所大厅 Phaser 场景（房间 + 患者小人走动/候诊），HUD 保留 React；点击患者进入现有对话流程 | 开罗式诊所大厅可玩 |
| B · 设施可视化 | 设施/升级在场景中体现（诊室/休息室外观变化），购置/升级点击交互 | 经营层与升级系统打通 |
| C · 对话开罗化 | 诊室场景 + 对话窗口开罗式呈现（保持文本可读性优先） | 对话流程开罗化 |
| D · 全界面统一 | 底部菜单栏、标题屏、成就图鉴、信纸、发现客户全部开罗式视觉统一 | 全界面风格一致 |
