...
| v1.0.0 | 2026-08-07 | 开罗风格界面彻底重构（§14，已立项）：grill-me 九项决策锁定（全量场景化/温暖手绘保留/Phaser+React 分工/大厅+诊室/点选导航/装修半自由/程序绘制/五期 M1-M5/逐句气泡+回顾）；独立文档体系 docs/kairosoft/{PRD,SPEC,PLAN}.md 完成；M1 技术地基动工 |

---

## 14. 开罗风格界面彻底重构（v1.0.0 · 已立项，M1 动工中）
...
>
> **grill-me 九项决策已锁定**：全量场景化（经营层+叙事层都进场景）｜温暖手绘风保留（非像素）｜Phaser 场景 + React UI 覆盖层｜大厅+诊室双场景｜医生可见+点选自动导航｜装修模式半自由摆放（位置仅视觉）｜全程序绘制（Phaser Graphics 按 palette 生成）｜五期 M1-M5｜对话面对面逐句气泡+右上角回顾窗。

### 14.1 目标与背景

将游戏界面从当前「网页卡片式」彻底重构为开罗（Kairosoft）模拟经营风格——参考《游戏开发物语》《温泉物语》《口袋学院物语》的布局、界面与交互范式。核心体验变化：

- 经营层从「列表 + 卡片」变为「可游走的 2D 场景」：患者在诊所里排队、走动、候诊
- 顶栏资源、操作入口、弹窗统一为开罗式视觉语言
- 原有玩法（对话诊疗 / 复诊 / 发现客户 / 回访 / 成就）**零变化**，只重做表现层

**技术选型结论：Phaser.js（不选 Three.js）。** 开罗是 2D 俯视/等距卡通像素风，Phaser 原生支持 2D sprite、动画、tilemap、场景管理，包体小、上手快；Three.js 是 3D WebGL 引擎，做 2D 开罗风格是杀鸡用牛刀，除非未来明确要 2.5D 等距 3D 表现，否则不引入。

### 14.2 开罗风格特征 → 本游戏映射

| 开罗特征 | 本游戏现状 | 开罗化方案 |
| --- | --- | --- |
| 顶栏资源条（金币/幸福度/天数） | 已有 HUD（等级/金钱/声望/理智） | 保留结构与 tooltip，视觉开罗化（资源图标 + 数值排布） |
| 中央经营场景（设施/人物游走） | 无，预约清单列表 | Phaser 场景：等候区/诊室/休息室/花园，患者小人排队、走动、到访 |
| 点击对象交互 | 点击列表卡片 | 点击场景中的患者/设施 → 弹出操作面板 |
...
| 画风 | 暖色手绘风（CSS Chibi） | **保持暖色手绘**，非像素；场景/设施/小人按 palette 程序绘制（用户决策） |
...
| 期 | 名称 | 交付物 |
| --- | --- | --- |
| M1 | 技术地基 | Phaser 集成、场景壳、EventBridge、医生小人点选导航走动、顶栏开罗化 |
| M2 | 经营层场景化 | 大厅全貌、设施可视化+升级面板+装修模式、患者候诊小人、底部菜单栏、网格导航 |
...
1. 经营层场景患者小人可游走/排队，点击可交互
2. 全界面视觉统一为开罗风格（顶栏/场景/菜单/弹窗）
