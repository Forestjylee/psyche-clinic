...
| 2026-08-09 | 移动端自适应（iPhone 优先） | 对话字号 cqw 套 clamp 保底（说话 ≥15px/旁白 ≥14px/名牌 ≥12px/气泡 max-width 260px）；HUD 手机竖屏（≤540px）单行折叠（品牌隐藏、等级+经验条/声望收进「⌄ 更多」、按钮图标化、展开态恢复进度条）；结算面板自适应高度（`.ending-card` 移除固定高度卡内滚动改 overlay 整层滚动，线框完整圈住内容）。Playwright 实测 iPhone 16 Pro 393×852 + 桌面回归（SPEC v1.9.0 / PLAN v2.7.0） |
| 2026-08-09 | 移动端二轮（iPhone 实测） | 患者气泡横向延伸（根因：absolute+left 锚定 shrink-to-fit 约束为「舞台宽−left」仅 156px，max-width 无效；`width:max-content` 让气泡按 `clamp(300px,40cqw,440px)` 铺开、锚点 660→580，实测 156→300px、高度 323→192px、选项零重叠）；手机首页 header 压缩（h1 28→17px/副标题隐藏/chips 瘦身/`.scene.clinic` 提级覆盖 cinema column），header 96→56px 预约清单多 40px。测试 274 全绿 + tsc（SPEC v1.9.1 / PLAN v2.7.1） |
...
| v2.7.0 | 2026-08-09 | **移动端自适应·iPhone 优先（SPEC §13 v1.9.0）**：① 对话字号 clamp 保底——`.clinic-stage` 16:9 舞台在 iPhone 竖屏（393×852）cqw=3.93px，玩家说话 1.9cqw≈7.5px 不可读；所有 cqw 字号套 `clamp(下限, cqw, 上限)`（说话 `clamp(15px,1.9cqw,20px)`/旁白 `clamp(14px,1.7cqw,18px)`/气泡名·名牌 `clamp(12px,…)`/疲惫句 `clamp(12px,1.6cqw,15px)`），气泡宽度 `clamp(260px,32cqw,360px)`；桌面 cqw 主导不变；② HUD 手机竖屏（≤540px）单行折叠——`hud` 高 56px、品牌隐藏（960px 断点）、等级(Lv+经验条)/声望收进「⌄ 更多」折叠面板（`.hud-stat.collapsible` 默认收起、`.hud.expanded` 展开恢复 `.hud-stat-bar` 进度条）、保存/反馈/退出按钮图标化（`.hud-btn-text` 隐藏留图标）、时段胶囊瘦身、`hud-stats` 允许 flex-wrap；③ 结算面板自适应高度——`.ending-card` 移除 `max-height`+卡内 `overflow-y` 改 `margin:auto` 高度随内容（线框完整圈住全部内容），超高时 `.ending-overlay` 加 `overflow-y:auto` 整层滚动。Playwright 实测 iPhone 16 Pro（393×852）HUD 无溢出/折叠可用/字号 15px，桌面 1440×900 回归正常 |
| v2.7.1 | 2026-08-09 | **患者气泡横向延伸 + 手机首页压缩（SPEC §13 v1.9.1）**：① 患者气泡——根因 `.speak-bubble` absolute+left 锚定 shrink-to-fit 被约束为「舞台宽−left」（iPhone 右侧患者仅 156px），`max-width` 形同虚设；加 `width:max-content` + 患者专属 `max-width: clamp(300px,40cqw,440px)`，锚点 x 660→580 使加宽后完整落在舞台内；实测 156→300px、高度 323→192px、选项零重叠；② 手机首页——≤540px header 收成单行（h1 28→17px/副标题隐藏/chips 瘦身/padding 收窄），`flex-direction:row` 用 `.scene.clinic .clinic-header` 提级覆盖 cinema.css 的 900px column；`clinic-body`/`section-title` 收窄；实测 header 96→56px、预约清单多 40px。Playwright 实测 iPhone 16 Pro + 桌面 1440×900 回归，274 测试全绿 + tsc |
